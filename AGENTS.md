...
- Code examples must be illustrative rather than a step-by-step prescription of exact changes to this codebase unless requested. Avoid patches, diffs, exact file-by-file replacement instructions, or exhaustive implementation sequences.
...
There are exceptions to this rule, agents may specifically edit files under `./design/` that ARENT `solidified_features.md` for example dumping language design information into `language_design_seed.md`
The same exception applies to everything under `textmate/`